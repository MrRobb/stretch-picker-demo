...
class ArmCommandGroup(SimpleCommandGroup):
    def __init__(self, range_m=None, calibrated_retracted_offset_m=None, node=None):
        SimpleCommandGroup.__init__(
            self, "joint_arm", range_m, acceptable_joint_error=0.008, node=node
        )
    ...
    def update(self, commanded_joint_names, invalid_joints_callback, **kwargs):
        ...
        if (
            self.name in commanded_joint_names
            or self.wrist_extension_name in commanded_joint_names
        ):
            if (
                self.name in commanded_joint_names
                and self.wrist_extension_name in commanded_joint_names
            ):
                err_str = (
                    "Received a command for the joint_arm and wrist_extension joints. "
            ...
            if len(active_telescoping_joint_names) != 0:
                err_str = (
                    "Received a command for the joint_arm/wrist_extension joint and one or more telescoping_joints. "
                    "These are mutually exclusive options. The joint names in the received command = "
                    "{0}"
                ).format(commanded_joint_names)
                invalid_joints_callback(err_str)
        ...
        if self.active:
            ...
            if goal_pos is None:
                err_str = (
                    ...
                    len(point.positions),
                    self.name
                    if not self.is_named_wrist_extension
    ...
    def init_execution(self, robot, robot_status, **kwargs):
        if self.active:
            _, extension_error_m = self.update_execution(
                robot_status, force_single=True
            )
            robot.arm.move_by(
                extension_error_m,
    ...
    def update_execution(self, robot_status, **kwargs):
        ...
        force_single = (
            kwargs["force_single"] if "force_single" in kwargs.keys() else False
        )
        self.error = None
        if self.active:
            if success_callback and robot_status["arm"]["motor"]["in_guarded_event"]:
                success_callback(
                    "{0} contact detected.".format(
                        self.name
                        if not self.is_named_wrist_extension
                        else self.wrist_extension_name
                    )
            ...
            if force_single:
                ...
                if self.is_telescoping:
                    return [
                        (telescoping_joint, self.error / len(self.telescoping_joints))
                        for telescoping_joint in self.telescoping_joints
                    ]
                else:
                    return [
                        (self.name, self.error),
                        (self.wrist_extension_name, self.error),
                    ]
        ...
        if len(active_incrementing_joint_names) != 0:
            if robot_mode == "position":
                ...
                err_str = (
                    ...
                    "Current mode = {1}."
                ).format(active_incrementing_joint_names, robot_mode)
                invalid_joints_callback(err_str)
                return False
